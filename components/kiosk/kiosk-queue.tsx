...
export function KioskQueue({ eventId, initialQueue }: KioskQueueProps) {
  ...
  useEffect(() => {
    const unsubscribe = subscribeRealtime<EventQueuePayload>(
      `event:${eventId}:queue`,
      (payload) => {
        setQueue(payload);
      }
  ...
  const activeStations = queue.stations.filter(
    (station) => station.isActive && station.type === "SCREENING"
  ).length;
  const averageWaitMinutes =
    activeStations > 0
      ? Math.ceil(queue.stats.waiting / activeStations) * 12
      : queue.stats.waiting * 12;

  const targetUnits = queue.event.targetUnits;
  const progressRatio =
    targetUnits > 0 ? Math.min(queue.stats.done / targetUnits, 1) : 0;
      ...
      <main className="relative mx-auto flex w-full max-w-6xl flex-col gap-8 px-8 pb-16 pt-16">
        <header className="flex flex-col gap-3 text-slate-100">
          ...
          <p className="text-sm text-slate-300">
            Target {queue.event.targetUnits} bags · Waiting{" "}
            {queue.stats.waiting} · Active screening stations {activeStations}
          </p>
        </header>
          ...
          <Card className="border-none bg-white/5 backdrop-blur">
            <CardHeader>
              ...
              <CardDescription className="text-slate-300">
                Volunteers will call donors in this order. We update instantly
                as stations advance.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {nextFive.length === 0 ? (
                ...
                nextFive.map((entry, index) => {
                  const stationInfo = entry.stationId
                    ? stationMap.get(entry.stationId)
                    : null;
                  const peopleAhead = index;
                  const eta =
                    activeStations > 0
                      ? Math.ceil(peopleAhead / Math.max(activeStations, 1)) *
                        12
                      : peopleAhead * 12;
                  const checkinSource = entry.checkinAt ?? entry.slotTime;
                  return (
                    <div
                      key={entry.id}
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-right text-sm text-slate-300">
                         ...
                         <NoSSR>
                            <p>Checked in {formatTime(checkinSource)}</p>
                          </NoSSR>
                        </div>
                      </div>
                    </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm text-slate-300">
                  ...
                  <span className="font-semibold text-slate-100">
                    {queue.stats.done}/{queue.event.targetUnits} bags (
                    {progressPercent}%)
                  </span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm text-slate-200">
                <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                  ...
                  <p className="mt-1 text-xl font-semibold text-slate-50">
                    {formatEta(averageWaitMinutes)}
                  </p>
                  <p className="text-xs text-slate-400">
                    Across {queue.stats.waiting} donors in queue
                  </p>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                  <p className="text-xs uppercase tracking-wide text-slate-400">
                  ...
                  <p className="mt-1 text-xl font-semibold text-slate-50">
                    {
                      queue.stations.filter((station) => station.isActive)
                        .length
                    }
                  </p>
                  <p className="text-xs text-slate-400">
                    {queue.stations.length} configured
                  </p>
                </div>
              </div>
        ...
        <section className="grid gap-4 md:grid-cols-2">
          {queue.stations.map((station) => (
            <Card
              key={station.id}
              className="border-none bg-white/5 backdrop-blur"
            >
              <CardHeader className="flex flex-row items-start justify-between">
                <div>
                  <CardTitle className="text-slate-50">
                    {station.type === "SCREENING" ? "Screening" : "Donation"}{" "}
                    Station
                  </CardTitle>
                  <CardDescription className="text-slate-300">
